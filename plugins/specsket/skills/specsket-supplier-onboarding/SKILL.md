...
Call `specsket_get_capabilities` first. The active principal must be `platform_admin`, and both `supplier_onboarding.validate` and `supplier_onboarding.stage` must be enabled. Fetch `supplier_proposal@1`; do not reuse product fields for supplier identities.
