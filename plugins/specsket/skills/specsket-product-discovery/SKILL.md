...
6. Mark each expected property as `verified`, `not_found`, `not_applicable`, or `requires_vendor_confirmation`. “Not found” means not found in the recorded checked sources, not that the information does not exist.
7. Call `specsket_assess_specification_completeness` with the server-issued snapshot ID and digest, the complete evidence-backed observation set, and the ordered search trace. Preserve the returned assessment digest. Never submit client-created weights or replace the server profile.
...
When the user asks to stage a selected candidate, use `specsket-product-ingestion`. Carry forward the exact evidence catalog, search trace, server-issued profile snapshot ID and digest, complete property observations, deterministic assessment digest, and completeness gaps. Revalidate against the current schema and profile; discovery results are not a staging receipt. Staging remains review-only and never publishes or approves a product.
