...
For an official collection or catalogue URL, automatically inspect the supplied page and relevant manufacturer-controlled product pages and technical documents. Treat a requested product count as a parent-product cap by default and retain every documented in-scope orderable identity for each selected parent. An exact-item count uses `orderable_source_identity`; per-category caps use `category_occurrence` and keep duplicate category occurrences visible while submitting duplicate parents once. Use `all_discovered` when no product cap is requested. Operational crawl limits never silently narrow requested product scope. Build a source graph before deciding output record count, split only when a source identity axis or the live combination capability requires it, and create only documented combinations. Continue into `specsket-product-ingestion` only after the user asks to stage.
Before recommending topology, render the capability-advertised discovery coverage. When batch-wide scope is enabled, show every parent, category occurrence, typed official identity and disposition, split lineage, global reconciliation counts, completeness scope, and source-frontier status; `product-source-coverage@2` remains chunk-local later during ingestion. Legacy coverage `@1` is `Legacy scope unverified` and cannot prove parent completeness. A representative exact identity is valid only when explicitly requested and labeled partial. Do not recommend topology while the discovery receipt is incomplete, and never imply completeness while discovery is unresolved.
